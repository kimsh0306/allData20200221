...
public class BaseBallGameView extends JFrame {
	...
	//화면과 관련된 코드 추가 끝
	String result[] = null;
	//전역변수를 선언하여 문제를 해결할 수 있어요. VO라는 디자인패턴과 관련이 있는 내용.
	...
	public BaseBallGameView(String result[]) {
		this.result = result;//지역변수 result를 전역변수 result에 대입
		if(this.result==null) {
			this.result = new String[2];//null 전변에 대해서 인스턴스했다.
			this.result[0] = "";
			this.result[1] = "";
		}
...
///////////////////화면처리 시작/////////////////////
	...
	/* 이전 버전에서는 friday0131소스 이벤트처리를 직접하였다. -ActionListener했다.
	 * 
	 */
		//디폴트 생성자는 JVM이 만들어 줄 수 있어요. 왜냐하면 파라미터가 없기 때문이죠.
		...
		jbtn_dap.setBackground(new Color(0,180,0));
		jbtn_exit.setForeground(new Color(212,212,212));
		jbtn_exit.setBackground(new Color(50,50,20));
		jbtn_clear.setForeground(new Color(212,212,212));
