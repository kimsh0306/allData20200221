...
public class LoginForm implements ActionListener{
	...
	ResultSet rs = null;
	DBConnectionMgr dbMgr = DBConnectionMgr.getInstance();//friendly상태는 서로 다른 패키지 접근 불가
	JFrame jf_login 	= new JFrame();
	JPanel jp_first 	= new JPanel();
	//jp_first속지 안에 서쪽에 jlb_id,중앙에 jtf_id
	...
	JButton jbtn_login	= new JButton(" 로그인 ");//로그인을 거쳐야 mem_id를 가져올 수 있다.
	//생성자
	LoginForm(){
		initDisplay();
	}
...
//왜냐하면 리턴타입을 String에서 String[]로 바꾸었고 그 바뀐 정보를 if문에서 
//비교해야 하니까 더이상 mem_name에 값은 if문에서 역할이 없어진 것이죠.
//비밀번호가 맞지 않습니다.를 result[0]에 담아주어야 합니다.
				if(mem_name.equals("0")) {//조회 결과가 있는거야?
					result[0] = "비밀번호가 맞지 않습니다.";//0번방에 이름이나 메시지를 넣는다.
				}
			}
			...
			//힌트 속에서 해결 방법을 찾을 수 있다.
			//에러 메시지를 JVM이 stack영역에 관리하는데 그 이력과 라인번호를 같이 출력해줌.
			e.printStackTrace();//기억해야한다.(디버깅)
...
//if문에서 return문을 만나면 그 메소드를 탈출 할 수 있다.
				return;
			}
			else {
